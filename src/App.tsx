...
import { createCliniAiccount, getAssessments, getCurrentUser, getSymptoms, saveAssessment, signIn, signOut, type SymptomCatalogItem } from "./services/supabase";
...
function App() {
  ...
  const [doctorProfile, setDoctorProfile] = useState<DoctorProfile>({ name: "Guest workspace", clinic: "CliniAi", email: "" });
  ...
  const profileFromUser = (user: { user_metadata?: { doctor_name?: string; clinic_name?: string }; email?: string | null }): DoctorProfile => ({
    ...
    clinic: user.user_metadata?.clinic_name || "CliniAi",
    email: user.email || "",
  });
  ...
  const generateRecommendations = async () => {
    ...
    try {
      ...
      setNotice(
        error instanceof Error
          ? error.message
          : "Unable to generate CliniAil recommendations.",
      );
    } finally {
      setBusy("");
    }
  ...
  const logout = async () => {
    ...
    try {
      ...
      setDoctorProfile({ name: "Guest workspace", clinic: "CliniAi", email: "" }); setSignedIn(false); setShowLanding(true);
    } catch (error) { setNotice(error instanceof Error ? error.message : "Unable to sign out. Please try again."); }
    finally { setLogoutBusy(false); }
  };

  if (!authReady) return <div className="app-loading" aria-live="polite">Loading CliniAi…</div>;
  ...
  return (
    <div className="app-shell">
      ...
      <aside className={mobileMenuOpen ? "mobile-drawer open" : "mobile-drawer"} aria-hidden={!mobileMenuOpen}>
        <div className="mobile-drawer-header"><strong>CliniAi</strong><button className="drawer-close" onClick={() => setMobileMenuOpen(false)} aria-label="Close menu"><X size={20} /></button></div>
      ...
      <main className="main">
        <header className="topbar">
          ...
          <button className="workspace-header-brand workspace-home-button" onClick={() => setShowLanding(true)} aria-label="Go to CliniAi landing page">
            <div className="landing-mark"><Stethoscope size={17} /></div>
            <strong>CliniAi</strong>
          </button>
          <button className="mobile-workspace-brand workspace-home-button" onClick={() => setShowLanding(true)} aria-label="Go to CliniAi landing page">
            <div className="landing-mark"><Stethoscope size={17} /></div>
            <strong>CliniAi</strong>
          </button>
          <div className="crumb">
        ...
        {view === "assessment" ? (
          <div className="content">
            <section className="page-heading">
              <div>
                <p className="eyebrow">CliniAiL WORKSPACE / 01</p>
                <h1>New patient assessment</h1>
                <p className="subheading">
                  AI-assisted CliniAil decision support for a more informed
                  diagnosis.
                </p>
              </div>
              <div className="secure">
                <ShieldCheck size={17} /> Secure CliniAil workspace
              </div>
            </section>
            {notice && (
            ...
            <section className="panel confirmation-panel">
              ...
              <div className="confirmation-grid">
                <div className="ai-callout">
                  ...
                  <span>Use CliniAil judgment to confirm or adjust.</span>
                </div>
            ...
            <section className="recommendation-bar">
              <div>
                ...
                <div>
                  <strong>Ready for CliniAil recommendations?</strong>
                  <span>
  ...
  return (
    <div className="landing-page">
      ...
      <main className="landing-hero">
        <div className="landing-badge"><Activity size={14} /> AI-assisted CliniAil support</div>
        <h1>More clarity for every<br /><em>CliniAil decision.</em></h1>
        <p>Enter a patient's details and symptoms. CliniAi helps you review possible conditions, organize suggested tests and medicines, and record the doctor's final decision.</p>
        ...
        <div className="landing-preview"><div className="preview-top"><span className="preview-dot" /><span>CliniAil workspace</span><span className="preview-status"><i /> System ready</span></div><div className="preview-body"><div><span className="tiny-label">TODAY'S WORKSPACE</span><strong>Patient assessment</strong></div><div className="preview-stats"><span><b>24</b> assessments</span><span><b>98%</b> reviewed</span></div><div className="preview-line" /></div></div>
      </main>
      <section className="landing-section feature-section" id="features"><div className="landing-section-intro"><p className="landing-kicker">ONE WORKSPACE, LESS FRICTION</p><h2>Built around the way CliniAil teams work.</h2><p>Keep symptoms, model insights, doctor confirmation, and recommendations in one calm, structured workflow.</p></div><div className="landing-feature-grid"><article><div className="feature-icon"><Activity size={18} /></div><h3>Structured assessments</h3><p>Capture patient details and symptoms consistently before prediction begins.</p></article><article><div className="feature-icon"><Microscope size={18} /></div><h3>Reviewable recommendations</h3><p>See suggested medications and tests clearly, then refine them as a clinician.</p></article><article><div className="feature-icon"><ShieldCheck size={18} /></div><h3>Records you can trust</h3><p>Keep the final doctor decision and feedback alongside the original AI insight.</p></article><article><div className="feature-icon"><Search size={18} /></div><h3>Focused CliniAil review</h3><p>Search symptoms quickly and keep the selected presentation visible throughout the assessment.</p></article><article><div className="feature-icon"><ClipboardCheck size={18} /></div><h3>Clinician-led confirmation</h3><p>Record the final diagnosis separately from the AI prediction to preserve CliniAil accountability.</p></article><article><div className="feature-icon"><Printer size={18} /></div><h3>Ready-to-share case sheets</h3><p>Generate a clear case record with the assessment, final recommendations, and submitted feedback.</p></article></div></section>
      <section className="landing-section process-section" id="how"><div className="landing-section-intro"><p className="landing-kicker">A CLEAR CliniAiL WORKFLOW</p><h2>From patient visit to a complete case sheet.</h2><p>CliniAi keeps the medical record, AI insight, and clinician decision in one reviewable workflow.</p></div><div className="workflow-demo" aria-label="CliniAil workflow demonstration"><div className="workflow-demo-top"><span>CliniAi WORKSPACE</span><span className="workflow-demo-status"><i /> Assessment in progress</span></div><div className="workflow-demo-steps"><div className="workflow-demo-card"><span className="workflow-demo-number">01</span><strong>Patient details</strong><small>Ananya Patel · PT-10482</small><div className="workflow-demo-tags"><b>Fever</b><b>Headache</b></div></div><ArrowRight className="workflow-demo-arrow" size={18} /><div className="workflow-demo-card"><span className="workflow-demo-number">02</span><strong>CliniAil review</strong><small>Possible condition</small><div className="workflow-demo-result"><Activity size={14} /><b>Dengue fever</b><span>82%</span></div></div><ArrowRight className="workflow-demo-arrow" size={18} /><div className="workflow-demo-card"><span className="workflow-demo-number">03</span><strong>Case sheet</strong><small>Doctor-confirmed plan</small><div className="workflow-demo-check"><Check size={14} /> Recommendations saved</div></div></div></div><div className="process-grid"><div><span>01</span><h3>Capture the CliniAil picture</h3><p>Add the patient's name, ID, age, and the symptoms observed during the consultation. This creates the context for the assessment.</p><small>Patient details and symptom list</small></div><div><span>02</span><h3>Review AI-supported possibilities</h3><p>Compare predicted conditions and confidence levels with your CliniAil judgment. AI suggests possibilities; the clinician remains the decision-maker.</p><small>Predictions, differential review, and clinician diagnosis</small></div><div><span>03</span><h3>Confirm, refine, and record</h3><p>Review recommended medicines and tests, make any needed changes, submit feedback, and generate a printable case sheet.</p><small>Final plan, feedback, and documented record</small></div></div></section>
      <section className="landing-section clinics-section" id="clinics"><div><p className="landing-kicker">FOR CLINICS AND CARE TEAMS</p><h2>Designed for thoughtful care at every scale.</h2><p>Give physicians and hospital staff a shared CliniAil workspace that keeps human judgment at the center.</p></div><button className="landing-cta large" onClick={() => onStart()}>Open the workspace <ArrowRight size={16} /></button></section>
      <section className="landing-section faq-section"><div className="landing-section-intro"><p className="landing-kicker">COMMON QUESTIONS</p><h2>Clear answers before you begin.</h2></div><div className="faq-list"><details><summary>Does CliniAi make the final diagnosis?</summary><p>No. CliniAi presents AI-supported possibilities and recommendations for review. The treating clinician remains responsible for the final diagnosis and plan.</p></details><details><summary>Where are assessment records stored?</summary><p>Signed-in clinic records are saved to the configured Supabase project, while each case sheet remains available for review and printing in the workspace.</p></details><details><summary>Who can use the workspace?</summary><p>CliniAi is designed for qualified clinicians and care teams who need a structured way to document and review patient assessments.</p></details></div></section>
      <section className="landing-final-cta"><div><p className="landing-kicker">READY WHEN YOU ARE</p><h2>Bring every assessment into one clear workspace.</h2><p>Create your clinic account to start documenting patient assessments, recommendations, and final CliniAil decisions.</p></div><div className="landing-final-actions"><button className="landing-cta large" onClick={() => { setAuthMode("signup"); setLoginOpen(true); }}>Create your clinic <ArrowRight size={16} /></button><button className="landing-login" onClick={() => { setAuthMode("login"); setLoginOpen(true); }}>Log in</button></div></section>
      <footer className="landing-footer"><div className="landing-brand"><div className="landing-mark"><Stethoscope size={17} /></div><strong>CliniAi</strong></div><span>CliniAil decision support for clinician-led care.</span><a href="mailto:support@CliniAi.app">support@CliniAi.app</a></footer>
      {loginOpen && <div className="login-overlay" role="dialog" aria-modal="true" aria-labelledby="login-title"><form className="login-card" onSubmit={submitLogin}><button type="button" className="login-close" onClick={() => setLoginOpen(false)} aria-label="Close login"><X size={18} /></button><div className="landing-mark"><Stethoscope size={19} /></div><p className="landing-kicker">CliniAi WORKSPACE</p><h2 id="login-title">{authMode === "signup" ? "Create your clinic" : "Sign in to your dashboard"}</h2><p>{authMode === "signup" ? "Set up CliniAi for your practice and keep every patient assessment organized." : "Access patient assessments, prediction history, and clinician feedback."}</p>{authMode === "signup" && <><label>Doctor name<input value={doctorName} onChange={(event) => setDoctorName(event.target.value)} placeholder="Dr. Rhea Sharma" /></label><label>Clinic name<input value={clinicName} onChange={(event) => setClinicName(event.target.value)} placeholder="Green Valley Clinic" /></label></>}<label>Email address<input type="email" autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="doctor@clinic.com" /></label><label>Password<input type="password" autoComplete={authMode === "signup" ? "new-password" : "current-password"} value={password} onChange={(event) => setPassword(event.target.value)} placeholder={authMode === "signup" ? "At least 6 characters" : "Enter your password"} /></label>{loginError && <div className="login-error"><AlertCircle size={15} />{loginError}</div>}<button className="landing-cta login-submit" disabled={loginBusy}>{loginBusy ? "Please wait..." : authMode === "signup" ? "Create clinic account" : "Sign in"} <ArrowRight size={15} /></button><small>{authMode === "signup" ? "Supabase may ask you to confirm your email before signing in." : "Use an account created in Supabase Authentication."}</small><button type="button" className="auth-switch" onClick={() => { setAuthMode(authMode === "signup" ? "login" : "signup"); setLoginError(""); }}>{authMode === "signup" ? "Already have an account? Sign in" : "New to CliniAi? Create your clinic"}</button></form></div>}
    </div>
  );
}
...
function WorkspaceView({ view, history, profile, onReturn }: { view: Exclude<View, "assessment">; history: AssessmentRecord[]; profile: DoctorProfile; onReturn: () => void }) {
  ...
  return (
    <div className="content workspace-view">
      <p className="eyebrow">CliniAiL WORKSPACE</p>
